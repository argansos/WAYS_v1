...
file_obj_setups = './configs/obj_setups.txt'
file_gof_setups = './configs/gof_setups.txt'
...
par_rix_deep = config['par_rix_deep']
...
def simu(coord):
    ...
    # replace the initial parameter with deeply fitted one
    if len(par_rix_deep) > 0:
        par[par_rix_deep] = par_replace_deep[:, i, j]
...
def run(time_s, time_e, module, rzsc_type, phenology='no', config_name='WAYS.txt', debug=[], coord=[]):
    ...
    if module == 'simu':
        ...
        # par_deep
        if len(par_rix_deep) > 0:
            file_par_deep = os.path.join(path_cf, 'par_deep_' + rzsc_type + '_' + phe_type + '.nc4')
            ds_par_deep = xr.open_dataset(file_par_deep)
            var_list = [var_name for var_name in ds_par_deep.data_vars]
            par_replace_deep = np.empty([len(var_list), len(ds_par_deep.lat.values), len(ds_par_deep.lon.values)])
            for k in range(len(var_list)):
    ...
    ncfiles = toolkit.namestr(var_id, file_syb, f_name, year_s, year_e, step=step)
...
if __name__ == '__main__':
    ...
    # run('2001-01-01', '2010-12-31', 'simu', 'cru')

    run('2008-01-01', '2008-12-31', 'simu', 'cru', phenology='no', config_name='WAYS1.txt')
